...
@interface MVVideoDetails ()
@property (strong , nonatomic) AVPlayerDemoPlaybackViewController *playbackViewController;
@property (nonatomic) BOOL    tapType;      //是否显示标题栏

@end

@implementation MVVideoDetails
...
- (void)viewDidLoad {
    ...
    //通知退出播放
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(endPlayAction:) name:@"endPlayAction" object:nil];
    //通知状态栏显示、隐藏
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hiddenStatusAction:) name:@"hiddenStatusAction" object:nil];
...
//通知状态栏显示、隐藏
-(void)hiddenStatusAction:(id)sender{
...
-(void)playAction{
    ...
    if (!_playbackViewController){
        _playbackViewController = [[AVPlayerDemoPlaybackViewController alloc] initWithNibName:@"AVPlayerDemoPlaybackViewController" bundle:nil];
